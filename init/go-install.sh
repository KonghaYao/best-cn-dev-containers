...
function find_version_link() {
  version_regex=`curl -s --connect-timeout 15 -H 'Cache-Control: no-cache' https://go.dev/dl/|grep -w downloadBox|grep src|grep -oE '[0-9]+\.[0-9]+\.?[0-9]*'|head -n 1`
  file_name="go$version_regex.$platform.tar.gz"
  link_regex="golang/$file_name"
  go_website="https://mirrors.aliyun.com/golang/"

  latest_version_link="$go_website$(
    wget -qO- "$go_website/" | # get the HTML of golang page
      grep -o "$file_name" | # select installation links
